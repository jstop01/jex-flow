...
import { X, Save, Repeat2, ChevronDown } from 'lucide-react';

interface ForEachEditModalProps {
  isOpen: boolean;
  onClose: () => void;
  nodeId: string | null;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-[2px]">
      <div className="bg-white rounded-2xl shadow-2xl overflow-hidden border border-slate-200 flex flex-col" style={{ width: '700px' }}>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#cddbfd] bg-[#dce4fd]">
          <div className="flex items-center gap-3">
            <div className="p-2.5 bg-white/60 rounded-xl">
              <Repeat2 size={22} className="text-[#5277f7]" />
            </div>
            <div>
              <h2 className="text-lg font-bold text-slate-800">ForEach 반복문</h2>
            ...
            <div className="relative">
              <button
                className={`w-full text-left bg-white px-4 py-2.5 rounded-lg border-2 transition-all duration-200 ${
                  showDropdown1
                    ? 'border-fuchsia-500 shadow-[0_0_0_3px_rgba(217,70,239,0.15)]'
        ...
        <div className="flex justify-end gap-3 px-6 py-4 border-t border-slate-200 bg-slate-50/80">
          ...
          <button
            onClick={handleSave}
            className="flex items-center gap-2 px-6 py-2.5 text-sm font-medium text-white bg-fuchsia-500 rounded-lg hover:bg-fuchsia-600 transition-colors shadow-sm"
          >
            <Save size={16} />
            저장
          </button>
